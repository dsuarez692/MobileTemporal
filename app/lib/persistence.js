...
exports.logIn = function(data){
	...
	if(index > 0){
		...
		if(cacheUser[index].password == data.password){
			//Cargo usuario de la cache
			loadUser(cacheUser[index]);
			return true;
		}else{
			//Error
...
exports.changePassword = function(password){
	...
	if(index > 0){
		//Actualizo password
		cacheUsers[index].password = password;
...
exports.logOut = function(){
	...
	for(var i=0; i < colas.length; i++){
		if(colas[i].owner == userData.username){
			colas[i] = cola;
			encontrado = true;
		}
	}
	...
	Ti.App.Properties.setString('colasReportes', colas);
